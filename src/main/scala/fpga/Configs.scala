...
class WithSystemModifications extends Config((site, here, up) => {
  ...
  case BootROMLocated(x) => up(BootROMLocated(x), site).map { p =>
    // Use pre-built bootrom for VCU19P
    val freqMHz = (site(SystemBusKey).dtsFrequency.get / (1000 * 1000)).toLong
    val make = s"make -C ../tools/palladium/src/main/resources/vcu19p/sdboot PBUS_CLK=${freqMHz} bin"
    require (make.! == 0, "Failed to build bootrom")
    p.copy(hang = 0x10000, contentFileName = s"../tools/palladium/src/main/resources/vcu19p/sdboot/build/sdboot.bin")
  }
  case ExtMem => up(ExtMem, site).map(x => x.copy(master = x.master.copy(size = site(VCU118DDRSize)))) // set extmem to DDR size
  case SerialTLKey => Nil // remove serialized tl port
})

// DOC include start: AbstractVCU19P and Rocket
class WithVCU19PTweaks extends Config(
  // clocking
